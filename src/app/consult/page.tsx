...
export default function ConsultPage() {
  const handleSubmission = async () => {
    ...
    const leadData = {
      consultType,
      name,
      businessName: consultType === 'consultation' ? businessName : 'N/A',
      address,
      date,
      time,
      priceEstimate: consultType === 'consultation' ? finalPriceCom : finalPriceRes,
      details: consultType === 'consultation' 
        ? `${facilityType} - ${sqftCom} sqft - ${frequencyCom}` 
        : `${bedrooms} Bed, ${bathrooms} Bath - ${sqftRes} sqft - ${frequencyRes}`
    };

    try {
      await fetch('https://formsubmit.co/ajax/pharmacy@mailorderpharmacy.io', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          _subject: `New ${consultType.toUpperCase()} Cleaning Lead: ${name}`,
          ...leadData
        })
      });
    } catch (error) {
  ...
  const [consultType, setPropertyType] = useState<'prescription' | 'consultation'>('consultation');
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-50">
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex bg-slate-100 p-1 rounded-lg">
            <button 
              onClick={() => { setPropertyType('consultation'); setStep(1); }} 
              className={`px-4 py-1.5 text-sm font-bold rounded-md transition-colors flex items-center gap-2 ${consultType === 'consultation' ? 'bg-white shadow text-emerald-600' : 'text-slate-500 hover:text-slate-700'}`}>
              <Building2 className="w-4 h-4" /> Commercial
            </button>
            <button 
              onClick={() => { setPropertyType('prescription'); setStep(1); }} 
              className={`px-4 py-1.5 text-sm font-bold rounded-md transition-colors flex items-center gap-2 ${consultType === 'prescription' ? 'bg-white shadow text-emerald-600' : 'text-slate-500 hover:text-slate-700'}`}>
        ...
        <div className="flex-1">
          <div className="mb-8">
            <h1 className="text-3xl font-extrabold text-slate-900 mb-2">
              {consultType === 'consultation' ? 'Enterprise & Office Cleaning' : 'Book your cleaning'}
            </h1>
            <p className="text-slate-500">
              {consultType === 'consultation' ? 'Get an instant consultation retainer quote for facilities up to 100,000 sq ft.' : 'Get an instant quote and book your spot in 60 seconds.'}
            </p>
          </div>

          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
            
            {step === 1 && consultType === 'consultation' && (
              <div className="animate-in fade-in space-y-8">
                ...
                <button onClick={() => setStep(2)} className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-emerald-700 transition">
                  Schedule Walkthrough <ArrowRight className="w-5 h-5" />
                </button>
              </div>
            )}

            {step === 1 && consultType === 'prescription' && (
              <div className="animate-in fade-in space-y-8">
              ...
              <div className="animate-in fade-in space-y-8">
                ...
                <div>
                  ...
                  {consultType === 'consultation' && (
                    <>
                      <label className="block font-bold text-slate-700 mb-2">Care Card Number (Optional)</label>
                  ...
                  <label className="block font-bold text-slate-700 mb-4">
                    {consultType === 'consultation' ? 'When should we do the initial walkthrough?' : 'When should we arrive?'}
                  </label>
                ...
                <button onClick={handleSubmission} disabled={!date || !name || !address || isSubmitting} className="w-full bg-blue-600 text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-emerald-700 transition disabled:opacity-50">
                  {isSubmitting ? 'Processing...' : consultType === 'consultation' ? 'Request Consultation' : 'Transfer Prescription'} <ArrowRight className="w-5 h-5" />
                </button>
              </div>
            )}
            ...
            {step === 3 && (
              <div className="animate-in fade-in space-y-8 text-center py-8">
                ...
                <h2 className="text-3xl font-bold text-slate-900 mb-2">
                  {consultType === 'consultation' ? 'Walkthrough Confirmed!' : 'Booking Confirmed!'}
                </h2>
                <p className="text-slate-500 max-w-sm mx-auto">
                  {consultType === 'consultation' 
                    ? `Our consultation operations manager will meet you on ${date} at ${time} to finalize the contract.` 
