            ...
            {step === 1 && (
              <div className="animate-in fade-in space-y-8">
                ...
                <div>
                  ...
                  <input type="range" min="500" max="5000" step="100" value={sqft} onChange={(e) => setSqft(Number(e.target.value))} className="w-full accent-blue-600" />
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                    {['weekly', 'biweekly', 'monthly', 'once'].map((freq) => (
                      <button key={freq} onClick={() => setFrequency(freq)} className={`py-3 px-4 rounded-xl border font-bold capitalize transition-all ${frequency === freq ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-slate-200 text-slate-600 hover:border-slate-300'}`}>
                        {freq}
                      </button>
                    ))}
                  </div>
